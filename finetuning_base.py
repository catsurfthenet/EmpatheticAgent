...
# load dataset
dataset = build_pretrain_dataset(config, dataset_path=train_dataset_path, input_min_text_length=min_input_length,
                                 input_max_text_length=max_input_length, size=train_set_size)
dev_dataset = build_pretrain_dataset(config, dataset_path=dev_dataset_path, input_min_text_length=min_input_length,
                                     input_max_text_length=max_input_length, size=dev_set_size)
...
# add special tokens
emo_labels = ['sadness', 'disappointment', 'neutral', 'fear', 'nervousness', 'disapproval', 'realization', 'annoyance',
              'grief', 'approval', 'caring', 'remorse', 'disgust', 'desire', 'love', 'anger', 'embarrassment', 'joy',
              'admiration', 'relief', 'surprise', 'optimism', 'confusion', 'curiosity', 'amusement', 'excitement',
              'gratitude', 'pride']
...
reward_classifier = pipeline('text-classification', model=reward_model_id, tokenizer=reward_model_id, max_length=128,
                             truncation=True, top_k=None, device=0)  #
...
# change
empathy_classifier = pipeline('text-classification', model=empathy_model, tokenizer=empathy_tokenizer, max_length=512,
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        ...
        for q in range(len(query_tensors)):
            ...
            t_ids = \
            tokenizer.encode_plus(batch["target"][q], add_special_tokens=True, padding='max_length', truncation=True,
        ...
        if val:
            ...
            for dev_batch in eval_dataloader:
                ...
                target_ids = \
                tokenizer(target, add_special_tokens=True, return_tensors="pt", padding=True, truncation=True,
                ...
                # emo_loss, mean_emo_loss = get_js_distance(prompt_results, emo_results)
                dev_loss = weights[0] * nll_loss + weights[1] * div_loss + weights[2] * norm_sim_loss.to(
                    device)  # + weights[3] * num_short_resp
