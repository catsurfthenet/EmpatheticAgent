...
# define path
save_path_prefix = "DEV_reverse_js_ppl_toxic_w4-4-2" #"DEV-mimic-lr-6-ppl-toxic" #"pure-bleu"
load_path_prefix = "../"
# define weights
emp_weight = 0.4 #0
toxicity_weight = 0.4
...
class ScriptArguments:
    ...
    model_save_path: Optional[str] = field(
        default=f"./{save_path_prefix}-emo-toxic-ppl", # blenderbot-400M-distill-empathy-score-only
...
ppo_trainer = PPOTrainer(
    ...
    optimizer=optimizer,
    num_shared_layers=4, # largest possible layer num = 11
)
...
with open(f'{save_path_prefix}_emo_probi_score_train_output.txt', 'w') as f:
    ...
    for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
        ...
        #target_tensors = response_tensors_t.clone()
        #loss0 = model(input_ids=response_tensors_t[0, :], labels=response_tensors_t[0, :])[1] # get loss
