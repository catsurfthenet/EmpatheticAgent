...
dev_set_size = 100
checkpoint = 50
epoch_num = 30 #1 # number of outer loops
shared_layers = 4
gamma = 1 #0.90
...
save_path_prefix = f"PPO{date.day}-{date.month}_ED_ts{train_set_size}_outer{epoch_num}_inner{ppo_epoch_num}_share{shared_layers}_scale{reward_scale}_empathy_clean_count0.4_emoDis{w[0]}_FACE{w[1]}_bertSim{w[2]}_{optimiser_choice}_bs{input_batch_size}_lr{lr}_gamma{gamma}" #"DEV_lr-7_ppl_toxic_w4-6-0" #"DEV-mimic-lr-6-ppl-toxic" # "DEV_SGD_lr-9_emo_toxic_w6-4-0"
load_path_prefix = ""
ppo_model = f"{load_path_prefix}FTM9-8-local_LinearLR_AdamW_wd0.001_vckp62_ts2000_bs16_lr5e-07_w0.5-1.0-1.5-0.0_FACE_norm_sim_loss_30epochs-1875-loss0.78064"
...
with open(f'{save_path_prefix}_score_train_output.txt', 'w') as f:
    for _ in range(epoch_num):
        for step, batch in tqdm(enumerate(ppo_trainer.dataloader)):
            ...
            div_reward = get_FACE_loss(token_freq=token_freq, outputs=response_tensors_t, special_ids=special_ids)
            #div_reward = [1/d for d in div_reward]

            current_match_reward += div_reward
            ...
            score_list = fluency_weight * inverse_ppl + score_list * emp_weight + div_weight * div_reward + norm_sim_reward * sim_weight #- 0.5 * float(nll_loss)
            #mean_emo_list.append(nll_loss)
            """
            for r in range(len(ref_resp_emo)):
                if emo_results[r][0]["label"] == ref_resp_emo[r]:
                    #if counter < use_target_steps:
