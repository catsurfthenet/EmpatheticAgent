...
val = True
num_epochs = 50
train_dataset_path = "modeldata/context_ws_empathy_clean_count_top10_score0.4_emo_train_ED_dataset.p"#sp_token_ws_empathy_clean_count_top10_score0.4_emo_train_dialogue_dataset.p"
dev_dataset_path = "modeldata/context_ws_empathy_clean_count_top10_score0.4_emo_validation_ED_dataset.p"#ws_empathy_clean_prompt_emo_validation_dialogue_dataset.p"
min_input_length = 20
...
weight_decay = 1e-3
checkpoint = 625 # once per 5 epoch
...
w = weights
model_save_path = f"no_sp_token_{num_epochs}epochs"#f"w{w[0]}-{w[1]}-{w[2]}-{w[3]}-{w[4]}_{num_epochs}epochs"#f"FTM15-8-local_conv_LinearLR_{optimiser_choice}_wd{weight_decay}_vckp{val_checkpoint}_ED_ts{train_set_size}_bs{train_batch_size}_lr{config.learning_rate}_w{w[0]}-{w[1]}-{w[2]}-{w[3]}-{w[4]}_FACE_norm_sim_loss_{num_epochs}epochs"
