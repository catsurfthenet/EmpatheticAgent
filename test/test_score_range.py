...
output_path_prefix = 'ppo_test_score_range'
load_path_prefix = '../'
cuda = False
score_min = 100
score_max = 0
...
empathy_classifier = load_empathy_classifier("../", cuda)
...
with open(f'{output_path_prefix}_text_log_emo_probi_score.txt', 'w') as text_log:
    ...
    toxic_results = toxicity_classifier(texts)
    #toxic_score = []

    for i in range(len(BLEU_score_list)):
        if toxic_results[i]["label"] == "toxic":
            toxic_score = 0
        else:
            toxic_score = toxic_results[i]["score"]
        temp_score = (list_emo_score[i] * emp_weight) + (BLEU_score_list[i] * fluency_weight) + (toxic_score * toxicity_weight)
