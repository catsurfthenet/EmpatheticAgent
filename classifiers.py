...
def compute_distances(sentence, dataset):
    """
    ...
    for index, _ in dataset.iterrows():
        df_s = dataset['query'][index]  #assuming the dataframe column is called 'sentences'
        distance = get_distance(df_s.lower(), sentence)
        distances.append(distance)
    return distances

def novelty_score(sentence, dataset):
    """
    ...
    if dataset.empty:
        ...
        d_list = compute_distances(sentence, dataset)
        d_score = sum(d_list)
        score = d_score / len(d_list)
    return round(score, 2)

def get_sentence_score(sentence, dataset):
    """
    ...
    novelty = novelty_score(sentence, dataset)
