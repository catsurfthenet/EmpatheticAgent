...
for repeat_num in range(total_repeat_num):
    ...
    model_save_path = f"ppo_empathy{repeat_num}"#script_args.model_save_path
    counter = 0
    best_score = 0
    mean_score_list = []
    mean_ppl_list = []
    ...
    with open(f'{save_path_prefix}_score_train_output.txt', 'w') as f:
        for _ in range(epoch_num):
            for ep in range(episode_num):
                ...
                score_list = fluency_weight * (-nll_loss) + emp_weight * np.array(score_list) + div_weight * (div_reward) + norm_sim_reward * sim_weight
