...
from helper import build_dataset, build_train_dataset, build_pad_dataset, get_mean, weighted_bleu_score, get_js_distance, emo_count_ppl_toxic, emo_dis_ppl, emo_dis_ppl_toxic, load_toxicity_classifier, load_empathy_classifier, load_emo_classifier, emo_dis_bleu, append_scores
...
input_batch_size = 24 # [8, 12, 16, 20, 24, 28]
optimiser_choice = "Adam" # ["Adam", "AdamW", "SGD"]
reward_function = emo_dis_ppl_toxic #[emo_dis_ppl_toxic, emo_count_ppl_toxic]
# define weights
emp_weight = 1 #0 # np.arange(0, 1, 0.1)
toxicity_weight = 0 # np.arange(0, 1, 0.1),  + emp_weight <= 1
fluency_weight = 0 #1 # 1 - emp_weight - toxicity_weight
ref_gen_emo_match_reward = 5 # np.arange(0, 9, 1)
reward_scale = 10 # np.arange(1, 10, 1)

lr = 1.47e-2
weight_decay = 0.001
ppo_epoch_num = 10
DEV = True
train_set_size = 3000 #3000
dev_set_size = 80
checkpoint = 50
...
save_path_prefix = f"test7_ep10_clean_emo_resp5xppl_ED_{optimiser_choice}_bs{input_batch_size}_lr-5_gamma0.99_emo_probi_w1-0-0" #"DEV_lr-7_ppl_toxic_w4-6-0" #"DEV-mimic-lr-6-ppl-toxic" # "DEV_SGD_lr-9_emo_toxic_w6-4-0"
load_path_prefix = "./"
...
dataset = build_train_dataset(config, dataset_path=train_dataset_path, input_min_text_length=min_input_length, input_max_text_length=max_input_length, size=train_set_size)
...
with open(f'{save_path_prefix}_score_train_output.txt', 'w') as f:
    for _ in range(epoch_num):
        for step, batch in tqdm(enumerate(ppo_trainer.dataloader)):
            ...
            #current_match_reward = current_match_reward / len(ref_resp_emo)
            if toxicity_weight > 0:
            ...
            """
            # add penalty
            for p in range(len(list_penalty)):
                score_list[p] -= list_penalty[p]
            """
            rewards = [torch.tensor(output) * reward_scale for output in score_list] # change reward
            ...
            if step % checkpoint == 0 and DEV:
                ...
                if ppo_trainer.accelerator.is_main_process:
                    ppo_trainer.save_pretrained(
                        f"{model_save_path}-step{step}-score{np.float32(current_score)}-ppl{np.float32(mean_ppl)}")
                f.write(f"\nSaving model at step {step}. \n")
