...
#change
finetune = True
ppo_model = "no_conv_50epochs-5250-loss1.26051"#"w1-1-1-1-0_50epochs-4250-loss1.32314"
text_generation_model = "ppo_model" # blenderbot, ppo_model, dialogpt, opt_iml

# set path prefix
output_path_prefix = f"{ppo_model}_bleurt_test"#f"FTM15-8_w5215_local_LLR_ED_ts2000_epoch10_sim_loss_bleurt_last_{text_generation_model}_test" #'DEV_cont3_emo_toxic-w6-4-0_bert_test'
...
# set bleurt checkpoint path
checkpoint = "../bleurt/BLEURT-20"

promcse_model = PromCSE("YuxinJiang/unsup-promcse-bert-base-uncased", "cls_before_pooler", 16)#.to(device)
...
# load test dataset
with open(f"{load_path_prefix}modeldata/sp_token_context_ws_empathy_clean_count_top10_score0.4_emo_test_ED_dataset.p", "rb") as f:
...
with open(f'{output_path_prefix}_text_log_emo_probi_score.txt', 'w') as text_log:
    ...
    for test_batch in enumerate(test_dataloader):
        ...
        input_conv_texts = [(context[i])+tokenizer.sep_token+(input_texts[i]) for i in range(len(input_texts))]
        #prompts = prompts + [(context[i].replace("</s>", "").replace("_comma_", ","))+tokenizer.sep_token+(input_texts[i].replace("</s>", "").replace("_comma_", ",")) for i in range(len(input_texts))]

        target = test_query["target"]
        targets_list = targets_list + [t.replace("</s>", "").replace("_comma_", ",") for t in target]
        input_ids = tokenizer(input_conv_texts, return_tensors="pt", padding='max_length', max_length=128, truncation=True).to(device)
        ...
        for g in range(len(generated_texts)):
            references = [target[g]]
            candidates = [generated_texts[g]]

            scorer = score.BleurtScorer(checkpoint)
            scores = scorer.score(references=references, candidates=candidates)
            assert isinstance(scores, list) and len(scores) == 1
            batch_bleurt_score.append(float(scores[0]))

        list_bleurt_score.append(get_mean(batch_bleurt_score))
    ...
    text_log.write(f"{counter} Mean Bleurt Score: {get_mean(list_bleurt_score)}, max: {max(list_bleurt_score)}, min: {min(list_bleurt_score)}.\n")
...
with open(f'{output_path_prefix}_test_score_log_emo_probi_score.txt', 'w') as score_log:
    ...
    score_log.write(f"{counter} Mean Bleurt Score: {get_mean(list_bleurt_score)}, max: {max(list_bleurt_score)}, min: {min(list_bleurt_score)}.\n")
